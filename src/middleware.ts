...
export async function middleware(request: NextRequest) {
  ...
  // 2. PROTEKSI RUTE RELAWAN (NATIVE AUTH) — dashboard, admin, dll
...
export const config = {
  matcher: ['/sma/:path*', '/relawan/:path*', '/dashboard/:path*', '/dashboard', '/admin/:path*', '/admin', '/reporting/:path*', '/reporting', '/laporan/:path*', '/laporan', '/input-grade/:path*', '/input-grade'],
};
