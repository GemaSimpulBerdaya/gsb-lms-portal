...
export default function SemestersPage() {
  ...
  const handleDeleteSemester = async (id: string) => {
    ...
    if (!confirm(`Hapus semester ${id}? Semua data (jadwal/laporan) tidak akan terhapus tapi tidak akan muncul di filter.`)) return;
  ...
  const [editModal, setEditModal] = useState<{ isOpen: boolean; oldName: string; newName: string }>({
    isOpen: false,
    oldName: "",
    newName: ""
  });
  ...
  const handleEditSemester = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const newList = semesters.map(s => s.name === editModal.oldName ? editModal.newName : s.name);
      
      // Also update active semester if the renamed one was active
      const activeSem = semesters.find(s => s.isActive)?.name;
      const body: any = { availableSemesters: newList };
  ...
  const [recapModal, setRecapModal] = useState<{ isOpen: boolean; data: SemesterData | null }>({
    isOpen: false,
    data: null
  });

  const handleCloseSemester = async (sem: SemesterData) => {
    if (!confirm(`Selesaikan semester ${sem.name}? Setelah diselesaikan, semester ini akan TERKUNCI PERMANEN dan tidak bisa diaktifkan kembali sebagai semester berjalan.`)) return;

    setSubmitting(true);
    try {
      // Get current closed list
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>Semester Center</h1>
          <p className={styles.subtitle}>Kelola timeline dan tinjau performa setiap semester.</p>
        </div>
                ...
                >
                  <td>
                    <div className={styles.semName}>
                      ...
                      {isPast && !sem.isActive && !sem.isClosed && <span className={styles.pastBadge}>Lampau</span>}
                    </div>
                  </td>
                  <td>
                  ...
                  <td>
                    <div className={styles.actions}>
                      {/* Tombol Aktifkan (hanya jika belum aktif & belum selesai) */}
                      ...
                      {/* Tombol Selesaikan (hanya jika aktif) */}
                      {sem.isActive && !sem.isClosed && (
                        <button 
                      ...
                      {/* Tombol Lihat Rekap (hanya jika sudah selesai) */}
                      {sem.isClosed && (
                        <button 
                      ...
                      {/* Edit hanya jika bukan masa lampau & belum selesai */}
                      {!isPast && !sem.isClosed && (
                        <button 
                          className={styles.editBtn}
                          onClick={() => setEditModal({ isOpen: true, oldName: sem.name, newName: sem.name })}
                          title="Edit Nama"
                        >
      ...
      {recapModal.isOpen && recapModal.data && (
        <div className={styles.overlay}>
          <div className={styles.modal} style={{ maxWidth: '600px' }}>
            ...
            <p className={styles.modalDesc}>Berikut adalah ringkasan performa LMS selama satu semester ini.</p>
      ...
      {isModalOpen && (
        <div className={styles.overlay}>
          <div className={styles.modal}>
            ...
            <p className={styles.modalDesc}>Gunakan format tahun (misal: 2025-1 atau 2025-Ganjil).</p>
